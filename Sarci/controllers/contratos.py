...
def contratos(PMaster, Padi, uou, n=10,):
    """
    ...
    try:
        ...
        return {"error": f"Erro na função Contratos: {str(e)}"}, None
...
def dea(arquivo):
    ...
    '''
    verifica = verificar(arquivo, colunas_obrigatorias=['Despes', 'Vlr. Emp. Líquido'], nome_do_arquivo='Relatorio de Destaques e Empenhos Analíticos', header=3)
...
def despesas(arquivo):
    ...
    '''
    verifica = verificar(arquivo, colunas_obrigatorias=['Descrição do Programa','Sd Dot.Atual', 'Emp. No Mês', 'Liq. No Mês'], nome_do_arquivo='Relatório Acompanhamento e Execução Orçamentaria', header=1)
    if verifica is not True:
        return verifica
    ...
    try: 
        ...
        return TabDespesadf.to_dict(orient='records')
    except Exception as e:
