...
def almoxerifado(arq): #palmo = planilha referente ao almoxerifado
    try:
        # ler tabela(s)
        todos_pdfs = [] #cria uma lista vazia onde será armazenados os caminhos relativos
 
        pagina_pdf = tabula.read_pdf(arq, pages='all', guess=False) #lê cada caminho relativo e transforma o pdf num dataframe
        ...
        result = df_almoxerifado.to_dict(orient='records')
        ...
        return jsonify({"error": f"Erro no processamento da função almoxerifado: {str(e)}"}), 500
...
def bensmoveis(ri):
   try:
        ...
        result = df_inventario.to_dict(orient='records')
        ...
        # Se ocorrer um erro, retorna uma resposta JSON com uma mensagem de erro e um código de status 500 (erro interno do servidor).
        return jsonify({"error": f"Erro no processamento da função bens moveis: {str(e)}"}), 500